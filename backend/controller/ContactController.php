<?php
  ...
  function addContact($data) {
    $newContact = Contact::ForInsert($data->creatorid, $data->clientid, $data->firstname, $data->lastname, $data->email, $data->phone, $data->maincontact);
    if($newContact->save()) {
  ...
  function updateContact($data) {
    ...
    if($contactToEdit->id == -1) { // we couldnt find the client
      ...
      $contactToEdit->phone = $data->phone;
